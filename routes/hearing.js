...
class StreamClient {
    constructor(config = {}) {
        ...
        logger.info('StreamClient initialized', {
            ...
            openAIEnabled: openaiService.enabled || false
        });
        
        console.log('🚀 [STREAM-CLIENT] Initialized with config:', {
            url: this.config.url,
            reconnectInterval: this.config.reconnectInterval,
            logDir: this.config.logDir,
            openAIEnabled: openaiService.enabled || false
    ...
    /**
     * Process message (called directly by StreamServer) - for server mode compatibility
     */
    ...
    async handleMediaEvent(message) {
        ...
        console.log(`🔍 [DEBUG] Current call: ${this.currentCall?.ucid}, Session exists: ${this.transcriptionSessions.has(ucid)}, Processor exists: ${this.audioProcessors.has(ucid)}`);
        ...
        if (!this.currentCall || this.currentCall.ucid !== ucid) {
            ...
            // Auto-create session if missing (for cases where START event was missed)
            if (!this.transcriptionSessions.has(ucid)) {
                console.log(`🔧 [AUTO-RECOVERY] Creating missing session for UCID: ${ucid}`);
                this.currentCall = {
                    ucid,
                    did: 'unknown',
                    startTime: new Date().toISOString(),
                ...
                await this.handleStartEvent({ ucid, did: 'unknown' });
            }
        }
        ...
        if (sampleRate === 8000) {
            ...
            if (this.currentCall.mediaPackets % 50 === 0 && samples && samples.length > 0) {
                console.log(`🎤 [TRANSCRIPTION-TRIGGER] Processing audio for UCID: ${ucid}, packet #${this.currentCall.mediaPackets}, samples: ${samples.length}`);
    ...
    async processAudioForTranscription(ucid, samples, sampleRate) {
        ...
        try {
            ...
            if (rms > MIN_SPEECH_RMS) {
                ...
                console.log(`🎙️ [TRANSCRIPTION] Speech detected for UCID: ${ucid}, RMS: ${rms.toFixed(2)}`);
            } else {
                ...
                if (silenceDuration >= 1000 && session.audioBuffer.length > 0) {
                    console.log(`🔇 [TRANSCRIPTION] 1-second silence detected, processing buffered audio for UCID: ${ucid}`);
            ...
            if (sessionDuration >= 5000 && session.audioBuffer.length > 0) {
                console.log(`⏰ [TRANSCRIPTION] 5-second timeout reached, processing buffered audio for UCID: ${ucid}`);
                await this.processBufferedAudio(ucid, session);
                return;
            }
    ...
    async processBufferedAudio(ucid, session) {
        if (!session.audioBuffer.length) {
            console.log(`📭 [TRANSCRIPTION] No audio buffer to process for UCID: ${ucid}`);
            return;
        }

        try {
            console.log(`🔄 [TRANSCRIPTION] Processing ${session.audioBuffer.length} samples for UCID: ${ucid}`);
            ...
            if (transcriptionResult && transcriptionResult.text && transcriptionResult.text.trim()) {
                ...
                console.log(`✅ [TRANSCRIPTION] Chunk processed for UCID: ${ucid} - "${chunk.text}"`);
                ...
                if (chunk.text.length > 10) { // Only interrupt for substantial input
                    console.log(`🛑 [TRANSCRIPTION] Interrupting playback for substantial input: ${chunk.text}`);
                    await this.handlePlaybackInterruption(ucid, chunk.text);
                }
            } else {
                console.log(`🔇 [TRANSCRIPTION] No transcribable speech found for UCID: ${ucid}`);
                logger.debug('No transcribable speech found', { ucid, bufferSize: session.audioBuffer.length });
            }
            
        } catch (error) {
            console.error(`❌ [TRANSCRIPTION] Error processing buffered audio for UCID: ${ucid}: ${error.message}`);
            ...
            console.log(`🧹 [TRANSCRIPTION] Audio buffer cleared for UCID: ${ucid}`);
        }
    }
    ...
    async handlePlaybackInterruption(ucid, transcribedText) {
        try {
            console.log(`🛑 [INTERRUPTION] User spoke during playback for UCID: ${ucid}: "${transcribedText}"`);
            ...
            console.error(`❌ [INTERRUPTION] Error handling interruption for UCID: ${ucid}: ${error.message}`);
